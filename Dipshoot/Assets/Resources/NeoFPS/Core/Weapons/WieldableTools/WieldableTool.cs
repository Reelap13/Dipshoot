...
namespace NeoFPS.WieldableTools
{
    ...
    public class WieldableTool : BaseWieldableItem, IWieldableTool, ICrosshairDriver, IPoseHandler
    {
        [SerializeField, FormerlySerializedAs("m_PrimaryAutoHold"), Tooltip("Do the primary actions play out in a sequence (eg play animation then heal) or are they active for the duration the button is held (eg grapple rope)")]
        private bool m_PrimaryIsSequenced = false;
        ...
        [SerializeField, FormerlySerializedAs("m_SecondaryAutoHold"), Tooltip("Do the secondary actions play out in a sequence (eg play animation then heal) or are they active for the duration the button is held (eg grapple rope)")]
        private bool m_SecondaryIsSequenced = false;
        ...
        public void PrimaryRelease()
        {
            ...
            if (primaryState == InputState.Held && m_PrimaryIsSequenced == false)
        ...
        {
            // Reset state if currently valid
            if (secondaryState == InputState.Held && m_SecondaryIsSequenced == false)
        ...
        protected override bool CheckIsBlocked()
        {
            return base.CheckIsBlocked() || isBusy;
        }
